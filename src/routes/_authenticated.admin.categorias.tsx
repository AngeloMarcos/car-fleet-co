...
function CategoriasPage() {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.from("categorias_veiculo").select("*").order("nome");
  ...
  async function save(e: React.FormEvent) {
    ...
    const { error } = edit
      ? await supabase.from("categorias_veiculo").update(payload).eq("id", edit.id)
      : await supabase.from("categorias_veiculo").insert(payload);
    if (error) return toast.error(error.message);
    toast.success(edit ? "Categoria atualizada." : "Categoria criada.");
    setOpen(false); load();
  }
  async function toggle(r: Row) {
    const { error } = await supabase.from("categorias_veiculo").update({ ativo: !r.ativo }).eq("id", r.id);
    if (error) return toast.error(error.message);
    load();
  }
